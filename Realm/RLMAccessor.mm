...
void kvoSetValue(__unsafe_unretained RLMObjectBase *const obj, NSUInteger index, ArgType value) {
    ...
    auto& prop = get_property(obj, index);
    // FIXME: name
    tracker.willChange(RLMGetObservationInfo(obj->_observationInfo, obj->_row.get_key(), *obj->_info), @(prop.name.c_str()));
    if constexpr (std::is_same_v<ArgType, RLMObjectBase *>) {
        tracker.trackDeletions();
    }
    setValue(obj, prop.column_key, static_cast<StorageType>(value));
}
        ...
        for (RLMProperty *prop in _info.rlmObjectSchema.properties) {
            ...
            if (prop.type == RLMPropertyTypeObject) {
                if (auto swiftAccessor = prop.swiftAccessor) {
                    [swiftAccessor set:(char *)(__bridge void *)objBase + ivar_getOffset(prop.swiftIvar) value:nil];
                }
